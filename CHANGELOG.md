...
### Fixed
- **Runs no longer stall on an unresponsive hazard API.** Every hazard lookup was retried in full (up to 20 s per CTX call, and up to four attempts per PubChem request), so a run with several hundred matches could spend hours waiting on a service that was down. Each external host now has a failure counter. After the first transport failure (timeout, connection error, or exhausted retries) the host is marked unavailable for the rest of the run, further requests return immediately, and the remaining compounds are reported as `Skipped (API unavailable)` while the run proceeds directly to the report. The same rule applies to the ctx-python client for EPA CompTox. HTTP 4xx responses are treated as answers, not failures, and do not trip the cut-off. The limit can be raised with the environment variable `K2_API_FAILURE_LIMIT` (0 disables the cut-off).
- The availability of each service at the end of the run is recorded in `run_manifest.json` under `external_apis`.

### Added
- `--no-hazard` skips all EPA CompTox and PubChem lookups, including structure images, for users who do not need the hazard columns or who know the services are unavailable.

---

## [3.1.0] - 2026-09-23

This release follows an independent review of v3.0.21 against the three papers the pipeline implements (Koelmel et al. 2022, Kwiecien et al. 2015, and the NYCSS manuscript with its supporting information). The review identifiers used below (S-* for scientific findings, D-* for software findings) refer to that report, and each finding is now covered by a test in the `tests/` package (`python -m pytest`, 99 tests). It should be noted that none of the Level 2 thresholds or windows were changed. Several of the fixes correct how a written criterion was being computed, however, and consequently they change scores. The "Migration" notes at the end of this entry describe which results are affected.

### Fixed (scoring)
- **Electron mass in the RHRMF (S-RHRMF-1).** The RHRMF compared measured ion m/z values against neutral sub-formula masses, whereas Kwiecien et al. subtract one electron mass before matching. The omission displaced the ±10 ppm window by 6 ppm at m/z 91 and by 11 ppm at m/z 51, so that perfectly measured light fragments were rejected and the effective tolerance was asymmetric (a toluene spectrum with a −5 ppm mass-axis error scored 0 while the same spectrum at +5 ppm scored 100). The window is now centered on the measured cation. RHRMF values increase for spectra dominated by fragments below roughly m/z 150, and the biphenyl features that scored 0 in the v3.0.20 diagnostic are a symptom of this defect.
- **Isotopically labeled formulas (S-RHRMF-2).** Deuterium and 13C symbols produced by molmass were dropped from the parent formula, so every labeled surrogate scored 0 against a low-resolution library entry. Atom masses are now taken from the molmass isotope table (which also removes a bias of roughly 0.8 ppm introduced by the previous five-decimal mass table), and formulas containing unrecognized atoms produce a warning at library load.
- **Exact-mass library detection (S-HR-1).** An entry is treated as exact-mass when its metadata says so (`resolution` or `hires` set to high) or when at least two of its peaks carry three or more decimal digits and one of those is among the three most intense. The previous rule, which was based on the size of the mass defect, classified exact-mass benzene, chlorobenzene, and dichlorobiphenyl entries as low-resolution and treated real C27 to C28 alkyl fragments as doubly charged artifacts. The previous rule is retained as `is_library_high_res_v3021()` for comparison.
- **Surrogate recoveries (S-SUR-1, S-SUR-2).** With internal-standard normalization enabled, the surrogate analyzer multiplied abundances that had already been normalized by the same factor a second time, so that a true recovery of 100% in a sample with a factor of 2 was reported as 200%. The surrogate matcher also now evaluates the 1.5% retention-index rule relative to the feature RI, as the screening engine does.
- **Unit-mass binning** rounds half-integer m/z values up consistently (76.5 → 77, 77.5 → 78) rather than using Python's round-to-even (S-DOT-2).

### Fixed (software)
- **Sample classification (D-1).** The per-sample Blank/Sample/Reference table written by the GUI was passed to `cli.py` as `--grouping` but never read, so the blank feature filter fell back to the blank-identifier substring regardless of what the user had entered. The grouping is now applied, and reference samples are taken from it.
- **Silent empty runs (D-2, D-5, S-BFF-1, S-RI-2).** A run with no quantification columns, no features, no sample columns, no blank columns, a missing calibration file, or MZmine data without a calibration file previously completed "successfully" with no matches. Each of these conditions now stops the run with a message that names the problem, and the final blank/sample classification is printed. `--allow-no-blanks` permits a run without blanks for the rare case in which that is intended.
- **One MSP reader (D-3).** Five separate MSP parsers, each accepting a different subset of the format, are replaced by `src/msp_reader.py`. It accepts a UTF-8 byte-order mark, CRLF line endings, header keys in any case, the `Retention_index:` and `NUM_PEAKS:` spellings, NIST peak lines of the form `41 59; 43 999;` and `(41 59)`, and records that are not separated by a blank line. A `Num Peaks` count that disagrees with the peak list produces a warning rather than a truncated spectrum. MZmine feature tables with `Peak height` or `datafile:X.mzML:area` columns are accepted, and sample names are normalized so that GUI file stems, `--reference-samples`, and column headers refer to the same column.
- **Calibration input (D-4).** A calibration table without a header no longer loses its first alkane, comma and semicolon delimiters are accepted, and duplicate carbon numbers or non-increasing retention times are rejected with an explicit message.
- **Internal-standard normalization (D-10)** is computed from the parsed abundances each time it runs, so repeating it within a session no longer scales abundances twice. Non-numeric IS values fail with a message, and the IS areas actually used are written to the `IS_Area_*` columns for the automatic methods.
- **Reports (D-9).** The `Instrument` and `Comments` columns now read the `instrument` and `comments` keys used by the templates. Reference samples are excluded from the feature-summary statistics (they were already excluded from the blank feature filter). A `Mean_Detected_Abundance` column is added. Hazard lookups are made once per CAS number, mirror plots are rendered only for the PDF, a single failed page no longer loses the whole PDF, and `temp_assets/` is removed at the end of a run unless `--keep-assets` is given.
- **External services (D-8).** All HTTP traffic goes through one session with timeouts, retries, and a rate limit for PubChem, and results are cached per CAS number in `~/.k2/cache/`. The guessed `EpaClient` endpoint, which returned an all-zero hazard matrix whenever an API key was present and thereby suppressed the PubChem fallback, is removed. DTXSIDs are resolved through ctx-python, and the pubchempy dependency is no longer needed.
- **GUI and pipeline (D-7).** The thread-count setting is honored (import serialization is now an explicit checkbox); the PyInstaller build runs the pipeline in-process rather than relaunching the GUI; `max_lib_peaks`, `ri_extrapolation`, and `allow_no_blanks` are exposed in the Analysis screen; project and preset state no longer carry over between projects; the EPA API key is passed through the `K2_EPA_API_KEY` environment variable and stored only in `~/.k2/credentials.json`; Tk widgets are updated from the main thread; Cancel stops MSConvert and MZmine as well as the Python process; an existing `results/<name>` folder is never overwritten (`_2`, `_3`, ... are appended); and the unused `main.py`, `InternalStandardScreen`, and `msdial_parser.py` are removed.

### Added
- **Retention-index provenance (S-RI-1).** Features that elute outside the alkane range are flagged `RI_Extrapolated = Yes` in every table, and a warning with the count is printed at load. `--ri-extrapolation {spline,linear}` selects how those RIs are derived. The default (`spline`) reproduces the previous behavior exactly, so published RIs are unchanged, while `linear` applies the van den Dool convention from the terminal alkane pair.
- **Run manifest (D-6).** Every run writes `run_manifest.json` with the K2 and package versions, the SHA-256 of every input file, all resolved options (including `max_lib_peaks`), the sample classification, library statistics, the calibration range, and feature and match counts. The pipeline log and the untruncated MZmine log are saved alongside it.
- **`HR_RevDot` and `HR_FwdDot` columns.** Exact-mass library matches report the 10 ppm peak-paired dot products they were gated on, and `RHRMF` reads `N/A` for them instead of the placeholder value 100.0 (S-HR-2).
- **Exit codes.** `cli.py` returns 0 when at least one match is found, 2 when the run completes with no matches (the summary tables and manifest are still written), and 1 on error. `--csv-only` and `--pdf-only` are mutually exclusive, and `--version` is available.
- **Tests and templates.** The `tests/` package builds a synthetic dataset from fragment formulas and checks every criterion, the parsers, and the command-line interface end to end. A `templates/ri_calibration_template.txt` alkane table is included so that the template data runs as shipped.
- **`docs/SCORING_METHODS.md`** describes each computation (weights, trimming, the exact-mass detector, the RHRMF, and extrapolation) in a form suitable for a methods section.

### Documentation
- The dot-product weighting (m/z¹ × I^0.5, squared cosine scaled to 1000), the trimming of library spectra to their 20 most intense peaks, and the 10 ppm re-scoring of exact-mass entries were previously undocumented and are now stated (S-DOT-1, S-TRIM-1). The `adjusted` blank-filter mode is labeled as an option outside the Koelmel framework, and the IS factor is documented as max(IS)/IS rather than the median.

### Migration
- Analyses in which RHRMF values, exact-mass classification, or surrogate recoveries matter should be re-run. RHRMF scores increase for light-fragment spectra, some candidates rejected by v3.0.x now pass, exact-mass aromatic and halogenated entries take the exact-mass path, and IS-normalized recoveries are lower by the IS factor.
- MZmine runs require `--ri-cal`, and runs without a blank column require `--allow-no-blanks`.
- `epa_client.py`, `msdial_parser.py`, and `main.py` are removed, and `pubchempy` is no longer a dependency.
