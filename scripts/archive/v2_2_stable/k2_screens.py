...
class BaseScreen(ttk.Frame):
    ...
    def _get_replicate_base_name(self, name):
        """Extract base name by stripping trailing A, B, C or _1, _2 suffixes"""
        import re
        # Match trailing A, B, C... or _1, _2...
...
class SampleGroupingScreen(BaseScreen):
    ...
    def __init__(self, parent, app):
        ...
        self.replicate_colors = [
            "#e6f3ff", "#fff2e6", "#f2ffe6", "#ffe6f2", "#f2e6ff", 
            "#e6fff2", "#fff9e6", "#e6f9ff", "#f9e6ff", "#f9ffe6"
        ]
        ...
        avg_check = ttk.Checkbutton(top_settings, text="Average replicates", variable=self.avg_reps_var)
        avg_check.pack(side='left')
        ...
        self.group_cb.pack(fill='x', pady=(0, 10))
    ...
    def on_show(self):
        ...
        self.avg_reps_var.set(self.app.pipeline_config.get('average_replicates', True))
        ...
        replicate_map = {} # sample_name -> replicate_id
        next_rep_id = 1
        ...
        for name in found_names:
            ...
            if stype == "Sample":
                base = self._get_replicate_base_name(name)
                if base not in base_names:
                    base_names[base] = []
                base_names[base].append(name)
        
        # Assign replicate IDs to groups with > 1 member
        for base, names in base_names.items():
            if len(names) > 1:
                for name in names:
                    replicate_map[name] = next_rep_id
                next_rep_id += 1
        ...
        try:
            for name in found_names:
                if name in existing_grouping:
                    ...
                    rep_id = replicate_map.get(name, 1) # Default to 1 if not a detected replicate
                    self.samples.append({
                        'name': name,
                        'type': stype,
                        'group': group,
    ...
    def refresh_table(self):
        ...
        for i, color in enumerate(self.replicate_colors):
            self.tree.tag_configure(f'rep{i+1}', background=color)
        ...
        for i, s in enumerate(self.samples):
            ...
            if s['type'] == 'Blank':
                ...
                # Use replicate ID to choose color
                color_idx = (s['replicate'] - 1) % len(self.replicate_colors)
                tags.append(f'rep{color_idx+1}')
                
            self.tree.insert('', 'end', iid=str(i), values=(s['name'], s['type'], s['group'], s['replicate']), tags=tags)
    ...
    def copy_to_selected(self):
        ...
        self.update_selected_replicate()
    ...
    def fill_down(self):
        ...
        try:
            for iid in selection[1:]:
                ...
                self.samples[idx]['replicate'] = template['replicate']
            self.refresh_table()
            self.tree.selection_set(selection)
        finally:
            self._updating_ui = False
    ...
    def on_double_click(self, event):
        ...
        elif column == "#4": # Replicate
            self._show_cell_entry(iid, column, x, y, w, h, "replicate")
    ...
    def _show_cell_entry(self, iid, column, x, y, w, h, key):
        ent = ttk.Entry(self.tree)
        ent.insert(0, str(self.samples[int(iid)][key]))
...
class ResultsScreen(BaseScreen):
    ...
    def on_match_select(self, event):
        ...
        avg_reps = self.app.pipeline_config.get('average_replicates', True)
        grouping = self.app.pipeline_config.get('sample_grouping', {})
        
        display_data = [] # List of (name, type, abundance)
        
        if avg_reps and grouping:
            # Group by replicate ID
            replicate_sets = {} # (type, group, rep_id) -> [val, ...]
            replicate_names = {} # (type, group, rep_id) -> [name, ...]
            
            for fld in abundance_fields:
                s_name = fld.replace('Abundance_', '')
                val = float(match.get(fld, 0))
                
                info = grouping.get(s_name, {})
                stype = info.get('type', 'Sample')
                sgroup = info.get('group', 'Group1')
                rep_id = info.get('replicate', 1)
                
                if stype == 'Blank':
                    # Blanks are typically not averaged unless requested, 
                    # but user said replicates are not relevant for blanks.
                    # Let's keep blanks as they are or average them into one if desired.
                    # For now, keep blanks individual but samples averaged.
                    display_data.append((s_name, "Blank", val))
                else:
                    key = (stype, sgroup, rep_id)
                    if key not in replicate_sets:
                        replicate_sets[key] = []
                        replicate_names[key] = []
                    replicate_sets[key].append(val)
                    replicate_names[key].append(s_name)
            
            # Build averaged display data
            for key, vals in replicate_sets.items():
                stype, sgroup, rep_id = key
                names = replicate_names[key]
                
                if len(names) > 1:
                    avg_val = sum(vals) / len(vals)
                    # Find common part
                    base_name = self._get_replicate_base_name(names[0])
                    if all(self._get_replicate_base_name(n) == base_name for n in names):
                        display_name = base_name
                    else:
                        display_name = f"RepGroup_{rep_id} ({sgroup})"
                    display_data.append((display_name, stype, avg_val))
                else:
                    display_data.append((names[0], stype, vals[0]))
        else:
            for fld in abundance_fields:
                s_name = fld.replace('Abundance_', '')
                val = float(match.get(fld, 0))
