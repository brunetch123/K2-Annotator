...
class SampleGroupingScreen(BaseScreen):
    ...
    def __init__(self, parent, app):
        super().__init__(parent, app)
        self.samples = []  # List of dicts: {'name': str, 'type': str, 'group': str}
        ...
        # Top panel for global settings
        top_settings = ttk.LabelFrame(content, text="Project Settings", padding=10)
        top_settings.pack(side='top', fill='x', pady=(0, 10))

        ttk.Label(top_settings, text="Number of Experimental Groups:").pack(side='left', padx=(0, 10))
        self.num_groups_var = tk.IntVar(value=2)
        self.num_groups_cb = ttk.Combobox(top_settings, textvariable=self.num_groups_var, values=[1, 2, 3, 4, 5, 6, 7, 8], width=5, state='readonly')
        self.num_groups_cb.pack(side='left', padx=(0, 20))
        self.num_groups_cb.bind('<<ComboboxSelected>>', lambda e: self.update_group_list())
        ...
        self.group_cb = ttk.Combobox(edit_panel, textvariable=self.group_var, state='readonly')
        self.group_cb.pack(fill='x', pady=(0, 20))
    ...
    def update_group_list(self):
        """Update the list of available groups based on num_groups_var"""
        count = self.num_groups_var.get()
        groups = [f"Group{i+1}" for i in range(count)]
        groups.append("Blank") # Always available for blanks
        self.group_cb['values'] = groups
        
        # Ensure current selection is valid
        if self.group_var.get() not in groups and self.group_var.get() != "":
    ...
    def on_show(self):
        ...
        if 'num_groups' in self.app.pipeline_config:
            self.num_groups_var.set(self.app.pipeline_config['num_groups'])
        ...
        try:
            for name in found_names:
                if name in existing_grouping:
                    ...
                    group = "Blank" if stype == "Blank" else "Group1"
                    self.samples.append({
                        'name': name,
                        'type': stype,
                        'group': group
    ...
    def refresh_table(self):
        ...
        for i, s in enumerate(self.samples):
            ...
            self.tree.insert('', 'end', iid=str(i), values=(s['name'], s['type'], s['group']), tags=tags)
    ...
    def update_selected_type(self, event=None):
        ...
        try:
            for iid in selection:
                ...
                # Force group to "Blank" if type is Blank
                if new_val == "Blank":
                    self.samples[idx]['group'] = "Blank"
            self.refresh_table()
    ...
    def update_selected_group(self, event=None):
        ...
        try:
            for iid in selection:
                idx = int(iid)
                # Don't allow changing group of a Blank unless changing type too
                if self.samples[idx]['type'] == "Blank":
                    self.samples[idx]['group'] = "Blank"
                else:
    ...
    def fill_down(self):
        ...
        try:
            for iid in selection[1:]:
                ...
                self.samples[idx]['group'] = template['group']
            self.refresh_table()
            self.tree.selection_set(selection)
        finally:
            self._updating_ui = False
    ...
    def on_double_click(self, event):
        ...
        if column == "#2": # Type
            ...
            self._show_cell_combobox(iid, column, x, y, w, h, self.group_cb['values'], "group")
    ...
    def _show_cell_combobox(self, iid, column, x, y, w, h, values, key):
        ...
        def finish(event=None):
            ...
            if key == "type" and val == "Blank":
                self.samples[int(iid)]["group"] = "Blank"
            cb.destroy()
            self.refresh_table()
    ...
    def go_next(self):
        ...
        self.app.pipeline_config['num_groups'] = self.num_groups_var.get()
...
class InternalStandardScreen(BaseScreen):
    ...
    def __init__(self, parent, app):
        ...
        ttk.Label(self.mz_ri_panel, text="Target m/z:", font=('Arial', 9, 'bold')).grid(row=0, column=0, sticky='w', pady=5)
        self.target_mz_var = tk.DoubleVar(value=0.0)
        ttk.Entry(self.mz_ri_panel, textvariable=self.target_mz_var, width=15).grid(row=0, column=1, sticky='w', padx=5, pady=5)

        ttk.Label(self.mz_ri_panel, text="m/z Tolerance:", font=('Arial', 9, 'bold')).grid(row=0, column=2, sticky='w', pady=5, padx=(15, 0))
        self.mz_tol_var = tk.DoubleVar(value=0.5)
        ttk.Entry(self.mz_ri_panel, textvariable=self.mz_tol_var, width=10).grid(row=0, column=3, sticky='w', padx=5, pady=5)

        ttk.Label(self.mz_ri_panel, text="Target RI:", font=('Arial', 9, 'bold')).grid(row=1, column=0, sticky='w', pady=5)
        self.target_ri_var = tk.DoubleVar(value=0.0)
        ttk.Entry(self.mz_ri_panel, textvariable=self.target_ri_var, width=15).grid(row=1, column=1, sticky='w', padx=5, pady=5)

        ttk.Label(self.mz_ri_panel, text="RI Tolerance:", font=('Arial', 9, 'bold')).grid(row=1, column=2, sticky='w', pady=5, padx=(15, 0))
        self.ri_tol_var = tk.DoubleVar(value=50.0)
        ttk.Entry(self.mz_ri_panel, textvariable=self.ri_tol_var, width=10).grid(row=1, column=3, sticky='w', padx=5, pady=5)
        ...
        ttk.Label(self.msp_panel, text="IS Spectrum File (.msp):", font=('Arial', 9, 'bold')).grid(row=0, column=0, sticky='w', pady=5)
        self.is_msp_var = tk.StringVar(value='')
        ttk.Entry(self.msp_panel, textvariable=self.is_msp_var, width=40).grid(row=0, column=1, sticky='ew', padx=5, pady=5)
        ttk.Button(self.msp_panel, text="Browse...", command=self.browse_is_msp).grid(row=0, column=2, sticky='w', pady=5)

        self.msp_panel.columnconfigure(1, weight=1)

        ttk.Label(self.msp_panel, text="RI Tolerance:", font=('Arial', 9, 'bold')).grid(row=1, column=0, sticky='w', pady=5)
        self.msp_ri_tol_var = tk.DoubleVar(value=50.0)
        ttk.Entry(self.msp_panel, textvariable=self.msp_ri_tol_var, width=10).grid(row=1, column=1, sticky='w', padx=5, pady=5)
    ...
    def go_next(self):
        ...
        if is_config['enabled']:
            ...
            if method == 'manual':
                ...
                target_ri = self.target_ri_var.get()

                if target_mz <= 0 or target_ri <= 0:
                    messagebox.showerror("Invalid Input", "Please enter valid m/z and RI values (> 0)")
                    return

                is_config['target_mz'] = target_mz
                is_config['mz_tolerance'] = self.mz_tol_var.get()
                is_config['target_ri'] = target_ri
                is_config['ri_tolerance'] = self.ri_tol_var.get()
                ...
                is_config['ri_tolerance'] = self.msp_ri_tol_var.get()
...
class ResultsScreen(BaseScreen):
    ...
    def __init__(self, parent, app):
        ...
        # Sub-notebook for different stats tools
        self.stats_notebook = ttk.Notebook(global_stats_tab)
        self.stats_notebook.pack(fill='both', expand=True, padx=5, pady=5)

        # Sub-tab 1: Volcano Plot
        self.volcano_tab = ttk.Frame(self.stats_notebook)
        self.stats_notebook.add(self.volcano_tab, text='Volcano Plot')
        
        # Volcano Controls
        v_ctrl = ttk.Frame(self.volcano_tab)
        v_ctrl.pack(fill='x', padx=5, pady=2)
        
        ttk.Label(v_ctrl, text="P-value Threshold:").pack(side='left', padx=5)
        self.p_thresh_var = tk.DoubleVar(value=0.05)
        self.p_entry = ttk.Entry(v_ctrl, textvariable=self.p_thresh_var, width=8)
        self.p_entry.pack(side='left', padx=5)
        
        ttk.Label(v_ctrl, text="Fold Change:").pack(side='left', padx=5)
        self.fc_thresh_var = tk.DoubleVar(value=2.0)
        self.fc_entry = ttk.Entry(v_ctrl, textvariable=self.fc_thresh_var, width=8)
        self.fc_entry.pack(side='left', padx=5)
        
        ttk.Button(v_ctrl, text="Update Plot", command=self.refresh_stats_plots).pack(side='left', padx=10)

        self.volcano_label = ttk.Label(self.volcano_tab, text="Generating Volcano Plot...")
        self.volcano_label.pack(fill='both', expand=True, padx=10, pady=2)

        # Sub-tab 2: PCA
        self.pca_tab = ttk.Frame(self.stats_notebook)
        self.stats_notebook.add(self.pca_tab, text='PCA Score Plot')
        self.pca_label = ttk.Label(self.pca_tab, text="Generating PCA Plot...")
        self.pca_label.pack(fill='both', expand=True, padx=10, pady=2)

        # Sub-tab 3: Heatmap
        self.heatmap_tab = ttk.Frame(self.stats_notebook)
        self.stats_notebook.add(self.heatmap_tab, text='Heatmap (Top 50)')
        self.heatmap_label = ttk.Label(self.heatmap_tab, text="Generating Heatmap...")
        self.heatmap_label.pack(fill='both', expand=True, padx=10, pady=2)

        # Sub-tab 4: Feature Boxplot
        self.boxplot_tab = ttk.Frame(self.stats_notebook)
        self.stats_notebook.add(self.boxplot_tab, text='Feature Detail')
        self.boxplot_label = ttk.Label(self.boxplot_tab, text="Select a feature from the list to see distribution")
        self.boxplot_label.pack(fill='both', expand=True, padx=10, pady=2)
    ...
    def apply_theme(self, theme_name):
        ...
        # Update sub-notebook style for theme
        style = ttk.Style()
        if theme_name == 'dark':
            style.configure("Stats.TNotebook", background="#2d2d2d")
        else:
            style.configure("Stats.TNotebook", background="#f0f0f0")
        
        self.stats_notebook.configure(style="Stats.TNotebook")
    ...
    def load_results(self):
        ...
        if csv_file and Path(csv_file).exists():
            try:
                ...
                # Initialize Stats and Refresh plots
                self.refresh_stats_plots()
    ...
    def refresh_stats_plots(self):
        """Re-calculate and re-draw all global statistics plots"""
        ...
        try:
            ...
            if not grouping:
                self.volcano_label.config(image='', text="Grouping Not Available\n(Ensure groups were assigned during setup)")
                self.pca_label.config(text="Grouping Not Available")
                self.heatmap_label.config(text="Grouping Not Available")
                return

            # Prepare Features from matches for interactive plotting
            from types import SimpleNamespace
            features = []
            for m in self.matches:
                f = SimpleNamespace()
                f.id = m.get('Feature_ID', '0')
                f.name = m.get('Compound_Name', 'Unknown')
                # Reconstruct abundances from match data
                f.abundances = {}
                for key, val in m.items():
                    if key.startswith('Abundance_'):
                        try:
                            s_name = key.replace('Abundance_', '')
                            f.abundances[s_name] = float(val)
                        except:
                            pass
                features.append(f)

            # Run Stats Engine
            stats_engine = StatsEngine(grouping)
            results = stats_engine.calculate_stats(features)
            
            # Generate and Show Plots
            # Volcano
            p_thresh = self.p_thresh_var.get()
            fc_thresh = self.fc_thresh_var.get()
            v_path = self.viz.create_volcano_plot(features, "Volcano Plot (Global)", "global_volcano_plot.png", p_threshold=p_thresh, fc_threshold=fc_thresh)
            self._display_image(v_path, self.volcano_label, (550, 400))

            # PCA
            p_path = self.viz.create_pca_plot(results['pca'], grouping, "PCA Score Plot", "global_pca_plot.png")
            self._display_image(p_path, self.pca_label, (550, 400))

            # Heatmap
            h_path = self.viz.create_heatmap(features, results['all_samples'], "Abundance Heatmap (Top 50)", "global_heatmap.png")
            self._display_image(h_path, self.heatmap_label, (650, 450))

        except Exception as e:
            print(f"Refresh Stats Error: {e}")

    def _display_image(self, path, label, size):
        """Helper to display image in label"""
        if path and Path(path).exists():
            try:
                ...
                self._img_refs[str(label)] = photo
            except Exception as e:
                label.config(image='', text=f"Error displaying plot: {e}")
        else:
            label.config(image='', text="Plot Not Available")

    def update_feature_boxplot(self, match):
        """Update the distribution boxplot for selected feature"""
        if not hasattr(self, 'viz') or not self.viz:
            return
        ...
        try:
            ...
            title = f"Distribution: {match.get('Compound_Name', 'Unknown')}"
            path = self.viz.create_feature_boxplot(f, groups, title, "current_feature_boxplot.png")
            self._display_image(path, self.boxplot_label, (450, 350))
    ...
    def on_match_select(self, event):
        ...
        # Update Feature Boxplot in Global Stats
        self.update_feature_boxplot(match)
