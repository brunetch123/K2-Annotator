...
class InternalStandardNormalizer:
    ...
    def normalize_auto_mz_ri(self, target_mz, mz_tolerance, target_ri, ri_tolerance):
        """
        Auto-detect IS feature by m/z and RI, then normalize.

        Args:
            target_mz: Target m/z value
            mz_tolerance: m/z tolerance (absolute)
            target_ri: Target RI value
            ri_tolerance: RI tolerance (absolute)

        Returns:
            tuple: (success: bool, feature_id: int or None)
        """
        # Find features matching m/z and RI criteria
        candidates = []
        ...
        for feat in self.features:
            ...
            if abs(feat_mz - target_mz) <= mz_tolerance:
                # Check RI match
                if abs(feat.ri - target_ri) <= ri_tolerance:
        ...
        if not candidates:
            print(f"[IS Normalizer] No features found matching m/z={target_mz}±{mz_tolerance}, RI={target_ri}±{ri_tolerance}")
            return False, None
    ...
    def normalize_auto_msp(self, msp_file_path, ri_tolerance=50):
        """
        Auto-detect IS feature by matching MSP spectrum, then normalize.

        Args:
            msp_file_path: Path to MSP file containing IS spectrum
            ri_tolerance: RI tolerance for matching (default: 50)

        Returns:
            tuple: (success: bool, feature_id: int or None)
        """
        # Parse MSP file to get spectrum and RI
        is_spectrum, is_ri = self._parse_msp_file(msp_file_path)

        if not is_spectrum:
            print(f"[IS Normalizer] Failed to parse MSP file: {msp_file_path}")
            return False, None

        print(f"[IS Normalizer] Loaded IS spectrum: {len(is_spectrum)} peaks, RI={is_ri}")
        ...
        for feat in self.features:
            ...
            # Check RI window (if RI available in MSP)
            if is_ri > 0:
                if abs(feat.ri - is_ri) > ri_tolerance:
                    continue
    ...
    def _parse_msp_file(self, msp_file_path):
        """
        Parse MSP file to extract spectrum and RI.

        Returns:
            tuple: (spectrum: list of (mz, intensity), ri: float)
        """
        spectrum = []
        ri = 0.0
        ...
        try:
            ...
            for line in lines:
                ...
                # Look for RI
                if line.upper().startswith('RETENTIONINDEX:') or line.upper().startswith('RI:'):
                    parts = line.split(':', 1)
                    if len(parts) == 2:
                        try:
