        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            for feat_id, candidates in self.results.items():
                ...
                for cand in candidates:
                    ...
                    # Get EPA toxicity info if API key provided
                    epa_link = ""
                    summary = ""
                    if self.api_key:
                        try:
                            from src.epa_client import EPAClient
                            epa = EPAClient(self.api_key)
                            hazard_data = epa.get_hazard_summary(cas if cas else inchikey)
                            if hazard_data:
                                summary = hazard_data.get('summary', '')
                                epa_link = hazard_data.get('dashboard_url', '')
                        except:
                            pass
