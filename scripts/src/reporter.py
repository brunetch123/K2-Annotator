...
class ReportGenerator:
    def __init__(self, results, features, output_dir="results", api_key=None, sample_columns=None, is_config=None,
        ...
        # v3.1.0 (D-9): reference (spiked) samples are excluded from the BFF
        # max-abundance by the engine, so exclude them from the summary
        # statistics too; otherwise the two tables disagree.
    ...
    @staticmethod
    def _meta(meta, *keys, default=''):
        """First non-empty metadata value among `keys` (v3.1.0, D-9: the
        templates ship 'instrument'/'comments' while older code read only
        'instrument_type'/'comments_raw')."""
        for k in keys:
            v = meta.get(k)
            if v not in (None, ''):
                return v
    ...
    def cleanup(self):
        """Remove the temp_assets/ folder (mirror plots, structure images)."""
        import shutil
        try:
            shutil.rmtree(self.viz.temp_dir, ignore_errors=True)
        except OSError:
            pass

    def _hazard(self, inchikey, name, cas):
        ...
        try:
            ...
            print(f"[WARNING] Hazard fetch error for {name}: {e}")
            res = ({}, ["Data Fetch Error"], f"https://comptox.epa.gov/dashboard/search/details?search={cas}")
        self._hazard_cache[cas] = res
        return res
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            for feat_id, candidates in self.results.items():
                ...
                for cand in candidates:
                    ...
                    if cas:
                        hazard_matrix, summary_list, epa_link = self._hazard(inchikey, comp.name, cas)
                        if summary_list:
                        ...
                        if not summary and hazard_matrix:
                            ...
                            summary = "; ".join(hazard_names) if hazard_names else "No GHS Data"
                        if not summary:
                            summary = "No Data"
                    else:
                        summary = "No CAS"
                    ...
                    # v3.1.0 (D-9): mirror plots are rendered by the PDF path only.
                    ...
                    row = [
                        ...
                        inchikey,
                        meta.get('source', ''),
                        self._meta(meta, 'instrument_type', 'instrument'),
                        self._meta(meta, 'comments_raw', 'comments', 'comment'),
                        epa_link,
    ...
    def generate_pdf(self, filename="Level2_Report.pdf"):
        ...
        for feat_id, candidates in self.results.items():
            ...
            for cand in candidates:
                ...
                # v3.1.0 (D-9): one bad image / lookup must not lose the
                # whole PDF; the page is skipped and the report continues.
                try:
                    ...
                    draw_kv("Instrument", self._meta(meta, 'instrument_type', 'instrument', default='N/A'))
                    # Removed Ion Mode as requested
                    draw_kv("Comment", str(self._meta(meta, 'comments_raw', 'comments', 'comment'))[:50])
                    ...
                    if not cas_val:
                        ...
                        # Fetch Matrix
                        matrix, summary, epa_url = self._hazard(meta.get('inchikey'), comp.name, cas_val)
                    ...
                    failed_pages += 1
                    print(f"[WARNING] Could not render PDF page for feature {feat_id} / {comp.name}: {e}")
                    try:
                        c.showPage()
                    except Exception:
                        pass
        if failed_pages:
            print(f"[WARNING] {failed_pages} match page(s) could not be rendered in the PDF")
